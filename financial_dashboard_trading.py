...
PRODUCTS = [
    {'label': '台積電: 2022.1.1 至 2024.4.9', 'file': 'kbars_2330_2022-01-01-2024-04-09.pkl', 'name': '台積電2330', 'start': '2022-01-01', 'end': '2024-04-09'},
    {'label': '大台指期貨2024.12到期: 2023.12 至 2024.4.11', 'file': 'kbars_TXF202412_2023-12-21-2024-04-11.pkl', 'name': '大台指期貨', 'start': '2023-12-21', 'end': '2024-04-11'},
    {'label': '小台指期貨2024.12到期: 2023.12 至 2024.4.11', 'file': 'kbars_MXF202412_2023-12-21-2024-04-11.pkl', 'name': '小台指期貨', 'start': '2023-12-21', 'end': '2024-04-11'},
    {'label': '英業達2020.1.2 至 2024.4.12', 'file': 'kbars_2356_2020-01-01-2024-04-12.pkl', 'name': '英業達2356', 'start': '2020-01-02', 'end': '2024-04-12'},
    {'label': '堤維西2020.1.2 至 2024.4.12', 'file': 'kbars_1522_2020-01-01-2024-04-12.pkl', 'name': '堤維西1522', 'start': '2020-01-02', 'end': '2024-04-12'},
    {'label': '台塑: 2022.1.1 至 2024.4.9', 'file': 'kbars_1301_20220101_20240409.pkl', 'name': '台塑1301', 'start': '2022-01-01', 'end': '2024-04-09'},
    {'label': '台泥: 2022.1.1 至 2024.4.9', 'file': 'kbars_1101_20220101_20240409.pkl', 'name': '台泥1101', 'start': '2022-01-01', 'end': '2024-04-09'},
    {'label': '和泰車: 2022.1.1 至 2024.4.9', 'file': 'kbars_2207_20220101_20240409.pkl', 'name': '和泰車2207', 'start': '2022-01-01', 'end': '2024-04-09'},
    {'label': '萬海: 2022.1.1 至 2024.4.9', 'file': 'kbars_2615_20220101_20240409.pkl', 'name': '萬海2615', 'start': '2022-01-01', 'end': '2024-04-09'},
    {'label': '陽明: 2022.1.1 至 2024.4.9', 'file': 'kbars_2609_20220101_20240409.pkl', 'name': '陽明2609', 'start': '2022-01-01', 'end': '2024-04-09'}
]
choice_labels = [p['label'] for p in PRODUCTS]
choice_label = st.selectbox('選擇金融商品', choice_labels, index=0)
choice_idx = choice_labels.index(choice_label)
product = PRODUCTS[choice_idx]
df_original = load_data(product['file'])
product_name = product['name']


# --- 1. 日期輸入 ---
start_date_str = st.text_input(f'輸入開始日期(日期格式: {product["start"]}), 區間:{product["start"]} 至 {product["end"]}', product["start"])
end_date_str = st.text_input(f'輸入結束日期 (日期格式: {product["end"]}), 區間:{product["start"]} 至 {product["end"]}', product["end"])
start_date = datetime.datetime.strptime(start_date_str, '%Y-%m-%d')
end_date = datetime.datetime.strptime(end_date_str, '%Y-%m-%d')

# --- 2. 去掉 DataFrame 'time' 欄位的時區 ---
if hasattr(df_original['time'], 'dt'):
    df_original['time'] = df_original['time'].dt.tz_localize(None)
...
if choice_label == choice_labels[0]:   ##'台積電: 2022.1.1 至 2024.4.9':
...
if choice_label == choice_labels[1]:   #'大台指期貨2024.12到期: 2023.12 至 2024.4.11':
...
if choice_label == choice_labels[2]:  #'小台指期貨2024.12到期: 2023.12 至 2024.4.11':
...
if choice_label == choice_labels[3]:   #'英業達2020.1.2 至 2024.4.12':
    交易總盈虧,平均每次盈虧,平均投資報酬率,平均獲利_只看獲利的,平均虧損_只看虧損的,勝率,最大連續虧損,最大盈虧回落_MDD,報酬風險比 = 計算績效_股票()
    # 交易總盈虧 = OrderRecord.GetTotalProfit()*1000          ## 取得交易總盈虧
    # 平均每次盈虧 = OrderRecord.GetAverageProfit()*1000         ## 取得交易 "平均" 盈虧(每次)
    # 平均投資報酬率 = OrderRecord.GetAverageProfitRate()    ## 取得交易 "平均" 投資報酬率(每次)  
...
if choice_label == choice_labels[4]:   #'堤維西2020.1.2 至 2024.4.12':
    交易總盈虧,平均每次盈虧,平均投資報酬率,平均獲利_只看獲利的,平均虧損_只看虧損的,勝率,最大連續虧損,最大盈虧回落_MDD,報酬風險比 = 計算績效_股票()


if choice_label == choice_labels[5]:   #'堤維西2020.1.2 至 2024.4.12':
    交易總盈虧,平均每次盈虧,平均投資報酬率,平均獲利_只看獲利的,平均虧損_只看虧損的,勝率,最大連續虧損,最大盈虧回落_MDD,報酬風險比 = 計算績效_股票()

if choice_label == choice_labels[6]:   #'堤維西2020.1.2 至 2024.4.12':
    交易總盈虧,平均每次盈虧,平均投資報酬率,平均獲利_只看獲利的,平均虧損_只看虧損的,勝率,最大連續虧損,最大盈虧回落_MDD,報酬風險比 = 計算績效_股票()

if choice_label == choice_labels[7]:   #'堤維西2020.1.2 至 2024.4.12':
    交易總盈虧,平均每次盈虧,平均投資報酬率,平均獲利_只看獲利的,平均虧損_只看虧損的,勝率,最大連續虧損,最大盈虧回落_MDD,報酬風險比 = 計算績效_股票()

if choice_label == choice_labels[8]:   #'堤維西2020.1.2 至 2024.4.12':
    交易總盈虧,平均每次盈虧,平均投資報酬率,平均獲利_只看獲利的,平均虧損_只看虧損的,勝率,最大連續虧損,最大盈虧回落_MDD,報酬風險比 = 計算績效_股票()

if choice_label == choice_labels[9]:   #'堤維西2020.1.2 至 2024.4.12':
    交易總盈虧,平均每次盈虧,平均投資報酬率,平均獲利_只看獲利的,平均虧損_只看虧損的,勝率,最大連續虧損,最大盈虧回落_MDD,報酬風險比 = 計算績效_股票()
...
#%%
##### 畫累計盈虧圖:
if choice_label == choice_labels[1] :     ##'台積電: 2022.1.1 至 2024.4.9':
    OrderRecord.GeneratorProfitChart(choice='stock',StrategyName='MA')
if choice_label == choice_labels[2] :                 ##'大台指期貨2024.12到期: 2023.12 至 2024.4.11':
    OrderRecord.GeneratorProfitChart(choice='future1',StrategyName='MA')
if choice_label == choice_labels[3] :                           ##'小台指期貨2024.12到期: 2023.12 至 2024.4.11':
    OrderRecord.GeneratorProfitChart(choice='future2',StrategyName='MA')
if choice_label == choice_labels[4] :                                       ##'英業達2020.1.2 至 2024.4.12':
    OrderRecord.GeneratorProfitChart(choice='stock',StrategyName='MA')
if choice_label == choice_labels[5] :                                                   ##'堤維西2020.1.2 至 2024.4.12':
    OrderRecord.GeneratorProfitChart(choice='stock',StrategyName='MA')
if choice_label == choice_labels[6] :                                                   ##'堤維西2020.1.2 至 2024.4.12':
